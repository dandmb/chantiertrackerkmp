...
class ProjectDetailHistoryAccessUiTest {
    ...
    @Test
    fun an_admin_can_open_the_modification_history() = runComposeUiTest {
            ...
            AppEnvironment {
                AppTheme {
                    Box(Modifier.size(412.dp, 892.dp)) {
                        ProjectDetailScreen("p1", viewModel = vm(currentUserIsOwner = true), onOpenHistory = { opened = it })
                    }
                }
            }
        }
    ...
    @Test
    fun a_supervisor_does_not_see_the_history_entry() = runComposeUiTest {
        setContent {
            customAppLocale = "fr"
            AppEnvironment {
                AppTheme {
        ...
        onNodeWithText("Historique des modifications").assertDoesNotExist()
    }
}
