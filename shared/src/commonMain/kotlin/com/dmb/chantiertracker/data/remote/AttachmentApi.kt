...
class AttachmentApi(private val client: HttpClient) {
    ...
    // The backend compresses server-side (resize ≤ 1600px + JPEG re-encode,
    // HEIC decode via ffmpeg — CONTEXTE.md §13 sexdecies); the client just
    // hands over the file bytes it picked, as-is.
    suspend fun upload(entryId: Long, bytes: ByteArray, fileName: String, mimeType: String): AttachmentDto =
